...
* 7) Export coefficients for figures (one CSV, includes excluded region label)
tempfile coef_all
preserve
clear
set obs 0
...
gen ci_upper        = .
gen model           = .
gen excluded_region = ""   // Africa, Americas, ...
gen outcome         = "PI0"
save `coef_all', replace
restore

* Helper: function to get coefficient positions robustly
program define _coefpos, rclass
    args bmat vmat varname
    tempname B V
    matrix `B' = `bmat'
    matrix `V' = `vmat'
    local pos = colnumb(`B', "`varname'")
