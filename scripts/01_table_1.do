...
* --- Cargar base ---
import delimited "`INFILE'", clear varn(1) encoding(UTF-8)

* Chequeos mínimos
capture confirm variable ssp
if _rc {
    di as error "Variable 'ssp' no encontrada (esperado: ssp126/ssp245/ssp585)."
    exit 111
}
foreach v in gdp country_standard totdeathspi totecost {
    capture confirm variable `v'
    if _rc {
        di as error "Variable '`v'' no encontrada en la base."
        exit 111
    }
}

* --- PIB global (sumando GDP por país, una sola vez) ---
preserve
keep if ssp=="ssp126"   // una copia por país alcanza
bys country_standard: keep if _n==1
summ gdp, meanonly
scalar G_WORLD = r(sum)
restore
...
gen pct_gdp    = 100 * totecost / G_WORLD
...
* Export CSV auxiliar
order ssp ssp_label totdeathspi losses_bil pct_gdp deaths_vs2008 loss_vs2013
...
quietly summarize pct_gdp if ord==1, meanonly
local G1 : display %6.3f r(mean)
quietly summarize pct_gdp if ord==2, meanonly
local G2 : display %6.3f r(mean)
quietly summarize pct_gdp if ord==3, meanonly
...
* --- Escribir LaTeX con file write (sin =string(...) en línea)
